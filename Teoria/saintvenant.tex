...
\section{Stato tensionale}
	...
	\subsection{Equilibrio indefinito, condizioni al contorno e legame costitutivo} \label{sec:sv:equilibrioindefinito}
...
\section{Analisi della flessione}
	...
	\subsection{Analisi della flessione deviata} \label{sec:fless:deviata}
...
\section{Analisi della torsione}
	...
	\subsection{Sezione circolare}
    	...
    	\figura{3.2}{2}{sv-statotorsione}{distribuzione del campo di tensione $\ov \tau$ in una sezione circolare piena.}{sv-statotorsione}
    ...
    \subsection{Rappresentazione dello stato di tensione}
    	...
    	\figura{4}{2}{sv-recip-polare}{stato di tensione in una trave sottoposta a momento torcente $M_z$ in un sistema di riferimento cilindrico; è possibile osservare, per reciprocità, l'esistenza della componente $\tzt$ dovuta a $\ttz$.}{sv-recip-polare}
    	...
    	\figura{6}{1}{sv-recip-rappresentazione}{rappresentazioni delle azioni agenti su un volume elementare nel sistema di riferimento cilindrico.}{sv-recip-rappresentazione}
    	
    	Usando le regole di trasformazione per rotazione dei tensori è possibile calcolare le componenti lungo gli assi $z',\theta'$ di un sistema di riferimento ottenuto per rotazione di un angolo $\phi$ rispetto al sistema $z,\theta$ iniziale (figura \ref{sv-recip-rappresentazione}):
    	\[ L = \begin{bmatrix}
    		\cos\phi & -\sin\phi \\ \sin\phi & \cos\phi
    	\end{bmatrix} \qquad \rightarrow \quad \S' = L^t \S L = \begin{bmatrix}
    		\ttz \sin(2\phi) & \ttz \cos(2\phi) \\ \ttz \cos(2\phi) & \ttz \sin(2\phi)
    	\end{bmatrix}\]
    	\figura{6}{1}{sv-recip-rotazione}{componenti di tensione ottenute dalla rotazione del sistema di riferimento iniziale.}{sv-recip-rotazione}
    	...
    	\figura{6}{1}{sv-torc-normale}{traiettorie principali di compressione e trazione per una trave sottoposta a momento torcente.}{sv-torc-normale}
    	...
    	Per fare una \textbf{verifica a rigidezza} scelto il gradiente di torsione ammissibile $\theta_{amm}$ agli estremi della trave è sufficiente verificare che
    ...
    \subsection{Sezioni compatte di forma qualsiasi}
    	...
    	\figura{5}{1.3}{effetto-torsione}{trave a sezione rettangolare prima (a sinistra) e dopo (destra) deformazione dovuta a momento torcente $M_z$.}{effetto-torsione}
    	...
    	In questo modo il \textbf{campo di spostamento} $\ov u = (u,v,w)$ è modellato dal gradiente di torsione $\Theta$ e dalla funzione di ingobbamento $\psi(x,y)$ e dunque
    	...
    	\subsubsection{Funzione delle tensioni}
    		...
    		In generale il calcolo della funzione $F$ in forma chiusa analitica è piuttosto complessa, quindi spesso si preferisce modellare il problema tramite delle analogie sperimentali.
    		...
    		\figura{6}{1}{sv-analogia-membrana}{membrana di sezione rettangolare uniformemente tesa ai bordi soggetta ad una pressione normale.}{sv-analogia-membrana}
    		...
    		\begin{subfigure}{0.48\linewidth}
    			\centering
    			\includegraphics[width=6.5cm]{sv-torsione-rett-a} \caption{}
    		\end{subfigure}
	    	\begin{subfigure}{0.48\linewidth}
		    	\centering
		    	\includegraphics[width=4.5cm]{sv-torsione-rett-b} \caption{}
		    \end{subfigure}
    		...
    		Facendo sempre riferimento alla distribuzione di tensione mostrata in figura \ref{fig:sv:tensione-torsione-rettangolo}.b è possibile misurare il modulo $\tau_{max}$ delle tensioni in corrispondenza delle estremità assiali (punti $A,B$) che può essere modellato tramite dei coefficienti $\alpha,\alpha'$ dipendenti dal rapporto $b/h$ (figura \ref{sv-coeff-tors}) delle dimensioni caratteristiche della sezione:
    		...
    		\figura{4.5}{2}{sv-coeff-tors}{coefficienti $\alpha,\alpha',\beta$ in funzione del rapporto $b/h$ delle dimensioni caratteristiche della sezione.}{sv-coeff-tors}
	...
	\subsection{Sezioni rettangolari a parete sottile e affini}
		...
		\figura{6}{1}{distribuzione-funzione-tensione}{funzione delle tensioni $F$ sui bordi di una sezione rettangolare a parete sottile.}{distribuzione-funzione-tensione}
    	...
    	\subsubsection{Sezioni "complesse" a parete sottile}
    		...
    		\figura{8}{0.6}{paretesottile}{esempio di sviluppo di una sezione ad "U" a parete sottile per giungere ad una sezione rettangolare "standard".}{paretesottile}
    	
    		Facendo riferimento alla sezione d'esempio si osserva che lo sviluppo determina una sezione rettangolare di base $a+2c$ e altezza $b$ e dunque il \textbf{fattore di rigidezza torsionale} $J_t$ associato alla sezione risulta valere
    		...
    		\figura{8}{1}{paretesottile-b}{esempio di sviluppo di una sezione circolare a parete sottile per giungere ad una sezione rettangolare "standard"; si osserva che la sezione non è composta da un pezzo unico.}{paretesottile-b}
    		...
    		\figura{4}{1.5}{angoli-torsione}{distribuzione di tensione in un angolo di una sezione a parete sottile.}{angoli-torsione}
    	...
    	\subsection{Sezioni chiuse a pareti sottili di forma qualsiasi}
    		...
    		\figura{4}{1.5}{sottile-chiusa}{rappresentazione schematica di una sezione chiusa a parete sottile e le grandezze necessarie alla sua descrizione.}{sottile-chiusa}
    		...
    		\figura{4}{1.5}{flusso-torcente}{forza $T$ dovuta al flusso della tensione e grandezze in gioco.}{flusso-torcente}
    		...
    		\paragraph{Seconda formula di Bredt} La seconda formula di Bredt si basa sull'analisi della deformata elastica, in particolare sul teorema di Clapeyron per il quale il lavoro delle azioni esterne $L_e = \frac 1 2 \Mz\Theta L$ deve essere uguale all'energia potenziale elastica $U_e$.
			...
			\figura{4}{1.5}{rettangolare-chiusa}{sezione rettangolare cava a pareti sottili.}{rettangolare-chiusa}    		
